...
import { getRequest, errorProvider, putRequest } from "../../../hooks";
...
const UpdateRole = () => {
  ...
  const { control, handleSubmit, register, setError, setValue } =
    useForm<RoleFormData>();

  useEffect(() => {
    const fetchTimeout = setTimeout(() => {
      if (id) {
        getRequest("/roles/permission-lists?role_id=" + id).then((res) => {
          setModules(res.data);
        });
        getRequest("/roles/" + id + "/get").then((res) =>
  ...
  return (
    <form onSubmit={handleSubmit(submitHandler)}>
      <h1 className="text-xl text-blue-500 font-semibold">EDIT ROLE</h1>
      <TextInput
        label="Name of Role"
        control={control}
        name="name"
