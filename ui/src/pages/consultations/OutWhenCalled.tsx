...
interface OutWhenCalledRowData {
  id: number;
  created_at: string;
...
const OutWhenCalled = () => {
  ...
  const { data, pagination, isFetching, changePage, isError } = useTable({
    endpoint: "consultations/out-when-called-consultations",
    pageSize: 10,
    parameters: debouncedParameters,
  });

  const columns: ColumnDefinition<OutWhenCalledRowData>[] = [
    {
