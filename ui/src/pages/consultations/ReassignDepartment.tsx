...
interface ReassignDepartmentRowData {
  created_at: string;
...
const ReassignDepartment = () => {
  ...
  const { data, pagination, isFetching, changePage, isError } = useTable({
    endpoint: "consultations/assigned-consultations",
    pageSize: 10,
    parameters: debouncedParameters,
  });

  const columns: ColumnDefinition<ReassignDepartmentRowData>[] = [
    {
    ...
    },
    {
      ...
      FilterComponent: (
        <FilterSelect
          ...
          name="department"
          onChangeFilter={(value) => {
            selectChangeHandler("department", value);
            changePage(1);
          }}
        />
      ),
...
export default ReassignDepartment;
